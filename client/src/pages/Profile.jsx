...
const Profile = () => {
  const { navigate, loadOrderData, orderData, token, currency, user, logout, wishlist,fetchWishlist } = useContext(ShopContext);
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="w-full md:w-1/3 flex flex-col gap-6">
            ...
            <div className="bg-white/90 dark:bg-gray-800/90 p-6 rounded-2xl shadow-2xl border border-primary/20 dark:border-secondary/20 flex flex-col items-center relative">
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-2 text-sm">{user?.email || "user@email.com"}</p>
              <button
            ...
            <div className="bg-white/90 dark:bg-gray-800/90 p-4 rounded-2xl shadow-2xl border border-primary/20 dark:border-secondary/20 flex flex-col">
              ...
              <ul className="space-y-1">
                {addresses.length === 0 ? (
                  ...
                  addresses.map(addr => (
                    <li key={addr.id} className="flex justify-between items-center bg-gray-50 dark:bg-gray-900 p-2 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800">
                      <span>
          ...
          <div className="w-full md:w-2/3 flex flex-col gap-8">
            ...
            <div className="bg-white/90 dark:bg-gray-900/90 p-6 rounded-2xl shadow-2xl border border-blue-100 dark:border-blue-900 flex-1">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-semibold text-xl flex items-center gap-2 text-blue-700 dark:text-blue-300">
                  <svg className="w-6 h-6 text-blue-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2" /><path d="M16 2v4M8 2v4M3 10h18" /></svg>
                  Recent Orders
                </h3>
                <button className="text-sm text-primary hover:underline" onClick={() => navigate("/orders")}>
                  View All Orders
                </button>
              </div>
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {orderDatas.length === 0 ? (
                  <li className="text-gray-400 text-sm text-center py-8 col-span-2">No orders yet.</li>
                ) : (
                  orderDatas.map(item => (
                    <li key={item.id} className="flex flex-col gap-2 bg-white/80 dark:bg-gray-900/80 p-4 rounded-xl shadow border border-blue-100 dark:border-blue-900 hover:shadow-lg transition">
                      <div className="flex items-center justify-between">
                        <span className="inline-block bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 font-mono text-xs px-3 py-1 rounded-lg shadow">{item.orderId}</span>
                        <span className="text-gray-700 dark:text-gray-200 font-medium">{new Date(item.date).toLocaleDateString()}</span>
                      </div>
                      <div className="flex flex-wrap items-center gap-2 justify-between">
                        ...
                        <span className="font-bold text-primary dark:text-blue-400">{currency}{item.amount}</span>
                        <span className="text-xs text-gray-500 dark:text-gray-400">Payment: {item.paymentMethod}</span>
            ...
            <div className="bg-white/90 dark:bg-gray-900/90 p-6 rounded-2xl shadow-2xl border border-pink-100 dark:border-pink-900 flex-1">
              <div className="flex justify-between items-center mb-4">
                ...
                <button className="text-sm text-primary hover:underline" onClick={() => navigate("/wishlist")}>
                  View All
                </button>
              </div>
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {wishlist.length === 0 ? (
                  <li className="text-gray-400 text-sm text-center py-8 col-span-2">No items in wishlist.</li>
                ) : (
                  wishlist.map(item => (
                    <li key={item.id} className="flex items-center justify-between gap-3 bg-white/80 dark:bg-gray-900/80 p-4 rounded-xl shadow border border-pink-100 dark:border-pink-900 hover:shadow-lg transition">
                      <span className="font-medium text-gray-800 dark:text-gray-100">{item.name}</span>
                      <span className="text-pink-600 dark:text-pink-400 font-semibold">{currency}{item.price}</span>
            ...
            <div className="w-full max-w-md mx-auto mt-8 bg-white/80 dark:bg-gray-900/80 rounded-2xl shadow-xl border border-primary/10 dark:border-secondary/20 p-5 text-center">
              <p className="text-primary dark:text-secondary font-semibold text-base">
                “Nutrio makes managing my orders and wishlist so easy. Love the experience!”
              </p>
              <span className="text-gray-500 dark:text-gray-400 text-xs">— Loyal Customer</span>
            </div>
          </div>
        </div>
      </section>
