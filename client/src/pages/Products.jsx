...
const Products = () => {
  const { categories, search, showSearch, products, category, setCategory  } = useContext(ShopContext)
  ...
  const applyFilter = () => {
    ...
    if (category.length > 0) {
      productsCopy = productsCopy.filter(item => category.includes(typeof item.category === "object" ? item.category._id : String(item.category)))
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-xl font-bold text-primary dark:text-secondary tracking-wide">Filters</h3>
            </div>
            <div className="space-y-8">
              {/* Category Filter */}
              <div>
                <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3 uppercase tracking-wider">Categories</h4>
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4 bg-white/70 dark:bg-gray-800/70 backdrop-blur-md  px-6 py-2 transition-all" >
              <p className="text-base font-semibold text-gray-800 dark:text-gray-100 tracking-wide">
                Showing <span className="text-primary dark:text-secondary">{filterProducts.length}</span> products
              </p>
              {/* sort section */}
